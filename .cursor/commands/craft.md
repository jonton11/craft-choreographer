...
2. **Act according to the current `phase`:**
   ...
   - **executing**: Use `executing_substep` in state. Default order per piece: `writer` → `chore` → `test_run`. Optional: insert **`review`** between writer and chore (see below). For the current piece (index `piece_index` from the `pieces` array):

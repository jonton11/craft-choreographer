...
3. **planning** → **awaiting_approval**: After Planner output is written to `plan_output`, set `phase: "awaiting_approval"`.
...
- **Agent prompts**: Receive placeholders filled from state (e.g. `{{initial_prompt}}`, `{{investigation_output}}`, `{{plan_output}}`, `{{current_piece}}`).
